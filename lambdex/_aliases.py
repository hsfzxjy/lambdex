    ...
    for name, value in aliases._asdict().items():
        ...
        if name[0].islower():
            ...
            if not value.isidentifier():
                raise ParsingError(
                    "alias for {!r} should be an identifier, got {!r}".format(
                        name, value
                    )
                )
            ...
            if iskeyword(value):
                raise ParsingError(
                    "alias for {!r} should not be a keyowrd, got {!r}".format(
                        name, value
                    )
                )
        ...
        # If `name` is an operator symbol
            ...
            if value not in COMPARATORS:
                raise ParsingError(
                    "alias for {!r} should be one of {}, got {!r}".format(
...
def get_aliases(userpaths=(), reinit=False) -> _Aliases:
    ...
    """
    global _aliases
    if _aliases is not None and not reinit:
        return _aliases
