...
def main():
    parser = argparse.ArgumentParser(
        "lxfmt-mock", description="Mock or reset specified formater backend"
    )
    parser.add_argument(
        "BACKEND",
    ...
    commands = [
        cmd
        for cmd in _whereis(command)
        if _is_generated_script(Path(cmd)) == opts.reset
    ]

    questions = [
        inquirer.List(
            "path",
            message="Which one do you want to {}?".format(
                "reset" if opts.reset else "mock"
            ),
            choices=commands,
        ),
    ]
